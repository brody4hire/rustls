...
mod tests {
    ...
    fn ticketrotator_switching_test() {
        let t = CfgX::new(crate::ticketer::TicketRotator::new(1, make_ticket_generator).unwrap());
    ...
    fn ticketswitcher_switching_test() {
        #[expect(deprecated)]
        let t = CfgX::new(crate::ticketer::TicketSwitcher::new(1, make_ticket_generator).unwrap());
        let now = UnixTime::now();
        let cipher1 = t.encrypt(b"ticket 1").unwrap();
        assert_eq!(t.decrypt(&cipher1).unwrap(), b"ticket 1");
        {
