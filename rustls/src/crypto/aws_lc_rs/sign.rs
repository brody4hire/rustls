...
pub fn any_ecdsa_type(der: &PrivateKeyDer<'_>) -> Result<CfgX<dyn SigningKey>, Error> {
    if let Ok(ecdsa_p256) = EcdsaSigningKey::new(
        der,
        SignatureScheme::ECDSA_NISTP256_SHA256,
        &signature::ECDSA_P256_SHA256_ASN1_SIGNING,
    ) {
        return Ok(CfgX::new(ecdsa_p256));
    }
...
pub fn any_eddsa_type(der: &PrivatePkcs8KeyDer<'_>) -> Result<CfgX<dyn SigningKey>, Error> {
    // TODO: Add support for Ed448
    Ok(CfgX::new(Ed25519SigningKey::new(
        der,
...
impl SigningKey for RsaSigningKey {
    fn choose_scheme(&self, offered: &[SignatureScheme]) -> Option<Box<dyn Signer>> {
        ALL_RSA_SCHEMES
            ...
            .map(|scheme| RsaSigner::new(Rc2::clone(&self.key), *scheme))
    }
...
impl RsaSigner {
    fn new(key: Rc2<RsaKeyPair>, scheme: SignatureScheme) -> Box<dyn Signer> {
