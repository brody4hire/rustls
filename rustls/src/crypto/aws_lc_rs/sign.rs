...
pub fn any_supported_type(der: &PrivateKeyDer<'_>) -> Result<Boxx<dyn SigningKey>, Error> {
    if let Ok(rsa) = RsaSigningKey::new(der) {
        return Ok(Boxx::new(rsa));
    }
...
pub fn any_ecdsa_type(der: &PrivateKeyDer<'_>) -> Result<Boxx<dyn SigningKey>, Error> {
    if let Ok(ecdsa_p256) = EcdsaSigningKey::new(
        der,
        SignatureScheme::ECDSA_NISTP256_SHA256,
        &signature::ECDSA_P256_SHA256_ASN1_SIGNING,
    ) {
        return Ok(Boxx::new(ecdsa_p256));
    }
...
pub fn any_eddsa_type(der: &PrivatePkcs8KeyDer<'_>) -> Result<Boxx<dyn SigningKey>, Error> {
    // TODO: Add support for Ed448
    Ok(Boxx::new(Ed25519SigningKey::new(
        der,
...
impl SigningKey for RsaSigningKey {
    fn choose_scheme(&self, offered: &[SignatureScheme]) -> Option<Box<dyn Signer>> {
        ALL_RSA_SCHEMES
            ...
            .map(|scheme| RsaSigner::new(Boxx::clone(&self.key), *scheme))
    }
...
impl RsaSigner {
    fn new(key: Boxx<RsaKeyPair>, scheme: SignatureScheme) -> Box<dyn Signer> {
