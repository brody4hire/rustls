...
impl Ticketer {
    ...
    #[cfg(feature = "std")]
    pub fn new() -> Result<Boxx<dyn ProducesTickets>, Error> {
        Ok(Boxx::new(crate::ticketer::TicketRotator::new(
            6 * 60 * 60,
    ...
    ) -> Result<Boxx<dyn ProducesTickets>, Error> {
        Ok(Boxx::new(crate::ticketer::TicketSwitcher::new::<M>(
            6 * 60 * 60,
            make_ticket_generator,
            time_provider,
        )?))
...
mod tests {
    ...
    fn ticketrotator_switching_test() {
        let t = Boxx::new(crate::ticketer::TicketRotator::new(1, make_ticket_generator).unwrap());
    ...
    fn ticketswitcher_switching_test() {
        #[expect(deprecated)]
        let t = Boxx::new(crate::ticketer::TicketSwitcher::new(1, make_ticket_generator).unwrap());
        let now = UnixTime::now();
        let cipher1 = t.encrypt(b"ticket 1").unwrap();
        assert_eq!(t.decrypt(&cipher1).unwrap(), b"ticket 1");
        {
